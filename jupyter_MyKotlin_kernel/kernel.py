...
class KotlinKernel(MyKernel):
    ...
    def do_runcode(self,return_code,fil_ename,class_filename,outpath,magics,code, silent, store_history=True,
        ...
        if magics['st']['runmode']=='repl':
            self._start_replprg('java',magics['st']['joptions']+[mainclass] + magics['st']['args'],magics)
        ...
        p = self.create_jupyter_subprocess(['java']+magics['st']['joptions']+[mainclass] + magics['st']['args'],env=self.addkey2dict(magics,'env'),magics=magics)
        self.subprocess=p
        self.g_rtsps[str(p.pid)]=p
        return_code=p.returncode
        bcancel_exec,retstr=self.raise_plugin(code,magics,return_code,fil_ename,3,2)
